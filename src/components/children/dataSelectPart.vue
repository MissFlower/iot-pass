<template>
  <div id="dataSelectPart">
    ...
    <div v-if="formData.dataType == 'int' || formData.dataType == 'float' || formData.dataType == 'double'">
    ...
    <div v-if="formData.dataType == 'enum'">
      ...
      <div class="df mt10 mb10">
        <div class="flex1">
          ...
          <el-tooltip>
            ...
            <div slot="content" class="f12 c9">支持整型，取值范围：-2147483648 ~ 2147483647</div>
          </el-tooltip>
        </div>
        <div class="flex1">
          参数描述
          <el-tooltip>
            <i class="el-icon-question c9"></i>
            <div slot="content" class="f12 c9">支持中文、英文大小写、数字下划线和短划线，必须以中文、英文或数字开头，不超过20个字符</div>
          </el-tooltip>
        </div>
      </div>
      <div v-for="(item, index) in formData.enumArr" :key="index" class="df enumItem">
        ...
        <el-form-item class="flex1">
          ...
          <el-button type="text" v-if="formData.enumArr.length > 1" class="ml2 hand" @click="delectEnum(index)">删除</el-button>
    ...
    <el-form-item label="布尔值" v-if="formData.dataType == 'bool'">
      <div>
        <span class="dib w30">0 -</span>
        <el-input v-model="formData.boolObj[0]" placeholder="如：关" class="ml20" style="width: calc(100% - 50px)"></el-input>
      </div>
      <div class="mt10">
        <span class="dib w30">1 -</span>
        <el-input v-model="formData.boolObj[1]" placeholder="如：开" class="ml20" style="width: calc(100% - 50px)"></el-input>
      </div>
    </el-form-item>
    <el-form-item label="数据长度" v-if="formData.dataType == 'text'">
      <el-input placeholder="请输入内容" v-model="formData.text">
        <template slot="append">字节</template>
      </el-input>
    </el-form-item>
    <el-form-item label="时间格式" v-if="formData.dataType == 'date'">
      <el-input disabled v-model="formData.dataText"></el-input>
    </el-form-item>
    <el-form-item label="JSON对象：" v-if="formData.dataType == 'struct'">
      <el-button type="text" icon="el-icon-plus" @click="addStruct">新增参数</el-button>
    </el-form-item>
    <div v-if="formData.dataType == 'array'">
      <el-form-item label="元素类型">
        <el-radio-group v-model="formData.arrObj.type">
          <el-radio v-for="(item, index) in arrTypes" :key="index" :label="item.value">{{item.text}}</el-radio>
        </el-radio-group>
      </el-form-item>
      <el-form-item label="元素个数">
        <el-input v-model="formData.arrObj.num" placeholder="请输入元素个数"></el-input>
      </el-form-item>
    </div>
    </el-form>
...
export default {
  ...
  data () {
    ...
    return {
      ...
      rules: {
        ...
        step: [
          {required: true, message: "000000", trigger: 'change' }
        ]
        // enumKey: [
        //   { required: true, message: "参数值不能为空", trigger: 'change' },
        //   {min: -2147483648, max: 2147483648, message: "支持整型，取值范围：-2147483648 ~ 2147483647", trigger: 'change' }
        // ]
      },
      unitArr: units
    }
  ...
  methods: {
    rangeValueFun (newVal) {
      let value = newVal
      if (this.formData.dataType === 'int') {
        value = newVal.replace(/\./g, '')
      } else {
        value = newVal.replace(".", "$#$").replace(/\./g, "").replace("$#$", ".")
        let len = 7
        if (this.formData.dataType === 'float') {
          len = 7
        } else {
          len = 16
        }
    ...
    handleChange () { // 选择数据类型
      switch(this.formData.dataType) {
        ...
        case 'double':
          ...
          break
        case 'enum':
          this.formData.enumArr = []
          this.addEnumItem()
          break
        case 'bool':
          this.formData.boolObj = {
            '0': '',
            '1': ''
          }
          break
        case 'text':
          this.formData.text = 2048
          break

      }
    },
    addEnumItem() {
      this.formData.enumArr.push({
        key: '',
        desc: '',
        errorKey: '',
        errorMenu: ''
    ...
    enumKeyChange (index) {
      const row = this.formData.enumArr[index]
      const enumKey = row.key
      if (enumKey === '') {
        row.errorKey = '参数值不能为空'
      } else {
        // row.error = Number.isInteger(enumKey * 1)
    ...
    delectEnum(index) {
      this.formData.enumArr.splice(index, 1)
    }
  }
}
</script>
