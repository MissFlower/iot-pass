...
<template>
  <div class="details">
    <div class="details-tit clearfix" v-if="JSON.stringify(batchDetailList) != '{}'">
      ...
      <el-tag :type="deviceType" class="el_tag" v-if="batchDetailList.taskStatus || batchDetailList.taskStatus == 0">{{taskStatusObj[batchDetailList.taskStatus].label}}</el-tag>
    </div>
    <div>
          ...
          <el-table :data="devManage.devList" border stripe>
            ...
            <el-table-column label="状态" prop="status">
              <template slot-scope="scope">
                ...
                <el-progress v-else :stroke-width="3" :percentage="scope.row.progress" :show-text="false" :color="color"></el-progress>
              </template>
            </el-table-column>
            <el-table-column label="操作" v-if="selectCountItemStatus != 3 || selectCountItemStatus != 4">
              <template slot-scope="scope">
