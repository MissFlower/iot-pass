<template>
  <div id="main" v-loading="loading">
      ...
      <el-row>
        <el-col :span="8">
          <div class="grid-content">
            <div class="grid_col grid_left">产品名称</div>
            <div class="grid_col grid_right">{{ productData.productName }}</div>
          </div>
        </el-col>
        <el-col :span="8">
          <div class="grid-content">
            <div class="grid_col grid_left">节点类型</div>
            <div class="grid_col grid_right">{{ nodeTypeData(productData.nodeType) }}</div>
          </div>
        </el-col>

        <el-col :span="8">
          <div class="grid-content">
            <div class="grid_col grid_left" style="word-break: break-all;">创建时间</div>
            <div class="grid_col grid_right">{{ productData.createTime }}</div>
          </div>
        </el-col>

        <el-col :span="8">
          <div class="grid-content">
            <div class="grid_col grid_left">所属品类</div>
            <div class="grid_col grid_right">{{ productData.categoryName || '-' }}</div>
          </div>
        </el-col>

        <el-col :span="8">
          <div class="grid-content">
            <div class="grid_col grid_left">数据格式</div>
            <div class="grid_col grid_right">{{ productData.dataFormat === 1 ? 'Json' : '透传/自定义' }}</div>
          </div>
        </el-col>

        <el-col :span="8">
          <div class="grid-content">
            <div class="grid_col grid_left">认证方式</div>
            <div class="grid_col grid_right">{{ authTypeData(productData.authType) }}</div>
          </div>
        </el-col>
        ...
        <el-col :span="8">
          <div class="grid-content">
            <div class="grid_col grid_left">状态</div>
            <div class="grid_col grid_right">{{ btnType === '' ? '发布' : '开发中' }}</div>
          </div>
        </el-col>

        <el-col :span="8">
          <div class="grid-content">
            <div class="grid_col grid_left">连网协议</div>
            <div class="grid_col grid_right">{{ netTypeData(productData.netType) }}</div>
          </div>
        </el-col>

        <el-col :span="btnType=='' ? 8 : 24">
          <div class="grid-content">
            <div class="grid_col grid_left">产品描述</div>
            <div class="grid_col grid_right">{{ productData.description || '-' }}</div>
          </div>
        </el-col>
      </el-row>
    </div>
    ...
    <el-dialog title="编辑产品信息" :visible.sync="dialogProEdit" width="30%" class="dialogProEdit">
      <el-form
        :model="dialogProEditForm"
        label-position="top"
        :rules="dialogProEditRole"
        ref="dialogProEditForm"
      >
...
<script>
import { dynamicRegisterSwitch, productEdit } from '@/api/product'
...
export default {
  ...
  methods: {
    ...
    dialogProEditData() {
      ...
      this.dialogProEditForm.dataFormat = (this.productData.dataFormat * 1 === 1 ? 'Json' : '透传/自定义')
